...
const ReceiptPdfGenerator = dynamic(() => import('@/components/ReceiptPdfGenerator'), {
  ssr: false,
  loading: () => (
...
function HomeContent() {
  ...
  const handleDraftEmail = () => {
    const subject = encodeURIComponent(`${invoice.fromName || 'Receipt'} - #${invoice.invoiceNumber}`);
    const body = encodeURIComponent(`Hi ${invoice.toName || 'there'},\n\nPlease find attached the receipt #${invoice.invoiceNumber} for ${invoice.currency} ${calculateTotal(invoice.items, invoice.taxRate, invoice.discountRate).toFixed(2)}.\n\nThank you for your business!\n\nBest regards,\n${invoice.fromName}`);
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* Form */}
      <h1 className="sr-only">Free Online Receipt Maker & Generator</h1>
          ...
          <div className={`space-y-4 ${activeTab === 'form' ? 'hidden lg:block' : ''}`}>
            <div className="sticky top-20">
              ...
              <div className="mt-4 flex flex-col gap-2">
                <ReceiptPdfGenerator data={invoice} theme={selectedTheme} />
                <button
